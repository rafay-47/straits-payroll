...
/// Attendance history provider with real-time updates
final attendanceHistoryProvider = StreamProvider<List<AttendanceModel>>((ref) async* {
  final userAsync = ref.watch(currentUserProvider);
  final user = userAsync.asData?.value;
  if (user == null) {
    yield [];
  ...
  try {
    yield* firestoreService.streamAttendanceByUser(user.uid);
  } catch (e) {
    print('Error fetching attendance history stream: $e');
    yield [];
  }
});

/// Today's check-in count for project (real-time)
final todayCheckInCountProvider = StreamProvider.family<int, String>(
  (ref, projectId) async* {
    final userAsync = ref.watch(currentUserProvider);
    final user = userAsync.asData?.value;
    if (user == null) {

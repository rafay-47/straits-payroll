...
class FirestoreService {
  ...
  Future<List<UserModel>> getPendingEmployees() async {
    try {
      ...
      // Super admin sees all pending employees
      if (role == 'superadmin') {
        print('🔑 User is SUPER ADMIN - fetching ALL pending employees');
        final snapshot = await _firestore
            .collection(AppConstants.usersCollection)
            .where('role', isEqualTo: AppConstants.roleEmployee)
        ...
        print('✅ Found ${snapshot.docs.length} total pending employees (super admin view)');
        return snapshot.docs.map((doc) => UserModel.fromMap(doc.data())).toList();
      }
      
      print('🏢 Fetching pending employees for COMPANY: $companyId');
      print('📋 Query Details:');
      ...
      print('   Total pending employees found: ${snapshot.docs.length}');
      
      // Filter by companyId (handles both old and new format)
      final filteredEmployees = <QueryDocumentSnapshot>[];
      
      for (var doc in snapshot.docs) {
        final data = doc.data();
        final employeeCompanyId = data['companyId'] as String?;
        final employeeName = data['name'] as String? ?? 'Unknown';
        final employeeId = data['employeeId'] ?? data['systemGeneratedId'] ?? 'N/A';
        final employeeStatus = data['status'] as String?;
        
        print('');
        print('   Checking: $employeeName ($employeeId)');
        print('      - UID: ${doc.id}');
        print('      - Employee CompanyId: "${employeeCompanyId ?? "NULL"}"');
        print('      - Admin CompanyId: "$companyId"');
        print('      - Status: $employeeStatus');
        
        // Match if companyId exactly matches
        if (employeeCompanyId == companyId) {
          print('      ✅ MATCH! Employee belongs to admin\'s company');
          filteredEmployees.add(doc);
        } else {
          print('      ❌ NO MATCH! Different company');
          print('         Employee: "${employeeCompanyId ?? "NULL"}"');
      ...
      print('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
      print('📊 FINAL RESULT:');
      print('   Total pending employees (all): ${snapshot.docs.length}');
      print('   Filtered for company "$companyId": ${filteredEmployees.length}');
      
      if (filteredEmployees.isEmpty) {
        print('');
        print('⚠️ NO PENDING EMPLOYEES FOUND FOR COMPANY "$companyId"!');
        print('');
        print('🔍 DIAGNOSTIC INFORMATION:');
        print('   1. Admin CompanyId: "$companyId"');
        print('   2. Total pending employees (all companies): ${snapshot.docs.length}');
        
        if (snapshot.docs.isNotEmpty) {
          print('');
          print('   Pending employees that exist (other companies):');
          for (var doc in snapshot.docs) {
            final data = doc.data();
            print('      - ${data['name']}: companyId="${data['companyId'] ?? "NULL"}"');
          }
          
          print('');
          print('   💡 POSSIBLE ISSUES:');
          print('      • Admin companyId ("$companyId") doesn\'t match any employee');
          print('      • Employees were created with different companyId');
          print('      • Check Firestore console to verify companyId fields');
        } else {
          print('   No pending employees exist in the entire system');
        }
      } else {
        print('');
        print('   ✅ Matching employees:');
        for (var doc in filteredEmployees) {
          final data = doc.data() as Map<String, dynamic>;
          print('      - ${data['name']} (${data['employeeId'] ?? data['systemGeneratedId']})');
        }
      }
      ...
      return filteredEmployees.map((doc) {
